package workflow

import (
	"bufio"
	"context"
	"errors"
	"os"
	ex "os/exec"
	"path/filepath"
	"runtime"
	"strings"
	"time"

	"github.com/hyprxlabs/go/dotenv"
	"github.com/hyprxlabs/go/env"
	"github.com/hyprxlabs/go/exec"
	"github.com/hyprxlabs/xtask/internal/schema"
	"github.com/hyprxlabs/xtask/internal/workflow/tasks"
	"gopkg.in/yaml.v3"
)

type Params struct {
	File                string
	Args                []string
	Tasks               []string
	Timeout             time.Duration
	CommandSubstitution bool
	Context             context.Context
	Command             string
	Dotenv              []string
	Env                 map[string]string
}

type WorkflowContext struct {
	Env     map[string]string
	Vars    map[string]interface{}
	Secrets map[string]string
	Targets schema.Hosts
	Timeout time.Duration
	Schema  *schema.Workflow
	Tasks   []schema.TaskDef
	Cwd     string
	Context context.Context
	Args    []string
	Config  *schema.Config
}

func Run(params Params) error {
	// Load the tasks file and parse it
	cleanUpEnv := false
	cleanUpPath := false
	tasksFile := params.File
	if !filepath.IsAbs(tasksFile) {
		absolute, err := filepath.Abs(tasksFile)
		if err != nil {
			return err
		}
		tasksFile = absolute
		params.File = tasksFile
	}

	bytes, err := os.ReadFile(tasksFile)
	if err != nil {
		return err
	}

	plan := &schema.Workflow{}
	if err := yaml.Unmarshal(bytes, plan); err != nil {
		return err
	}

	taskSet := []schema.TaskDef{}
	if len(plan.Tasks) > 0 {
		for _, task := range plan.Tasks {
			taskSet = append(taskSet, task)
		}
	}

	switch params.Command {
	case "ls":
		fallthrough
	case "list":
		max := 0
		for _, task := range taskSet {
			if len(task.Id) > max {
				max = len(task.Id)
			}
		}

		for _, task := range taskSet {
			id := task.Id
			desc := ""
			if task.Desc != nil {
				desc = *task.Desc
			}

			if len(desc) > 0 {
				// Pad the ID to align the output
				id = id + " "
				for len(id) < max {
					id += " "
				}

				os.Stdout.WriteString("\x1b[34m" + id + "- " + desc + "\x1b[39m\n")
			} else {
				os.Stdout.WriteString("\x1b[34m" + id + "\x1b[39m\n")
			}

		}

		return nil
	case "exec":
		execCommand(plan, params)
		return nil
	}

	cycles := findCyclicalReferences(taskSet)
	if len(cycles) > 0 {
		os.Stdout.WriteString("Cyclical references found in tasks:\n")
		for _, cycle := range cycles {
			os.Stdout.WriteString(" - " + cycle.Id + "\n")
		}

		os.Exit(1)
	}

	tasks := []schema.TaskDef{}
	if len(params.Tasks) == 0 {
		params.Tasks = []string{"default"}
	}

	tasks, err = flattenTasks(params.Tasks, plan.Tasks, tasks)
	if err != nil {
		return err
	}

	if len(tasks) == 0 {
		return errors.New("no tasks found to run")
	}

	ctx := WorkflowContext{
		Env:     make(map[string]string),
		Vars:    make(map[string]interface{}),
		Secrets: make(map[string]string),
		Targets: plan.Hosts,
		Context: params.Context,
		Args:    params.Args,
		Config:  plan.Config,
	}

	doc := dotenv.NewDocument()

	os.Chdir(filepath.Dir(tasksFile))
	mapEnv := env.All()
	mapEnv = normalizeEnvMap(mapEnv)
	mapEnv["XTASK_FILE"] = tasksFile
	mapEnv["XTASK_DIR"] = filepath.Dir(tasksFile)

	shell := "bash"

	if plan.Config != nil && plan.Config.Shell != "" {
		shell = plan.Config.Shell
	} else if runtime.GOOS == "windows" {
		shell = "powershell"
	}
	mapEnv["XTASK_SHELL"] = shell

	opts := &env.ExpandOptions{
		CommandSubstitution: params.CommandSubstitution,
		Env:                 mapEnv,
		Get: func(key string) string {
			if val, ok := mapEnv[key]; ok {
				return val
			}
			if val, ok := os.LookupEnv(key); ok {
				return val
			}
			return ""
		},
		Set: func(key, value string) error {
			mapEnv[key] = value
			return nil
		},
	}

	if plan.Config != nil {
		if len(plan.Config.Env) > 0 {
			for key, value := range plan.Config.Env {
				next, err := env.ExpandWithOptions(value, opts)
				if err != nil {
					return err
				}
				mapEnv[key] = next
			}
		}

		if len(plan.Config.PrependPaths) > 0 {
			for _, prependPath := range plan.Config.PrependPaths {
				path := prependPath.Path
				if strings.ContainsRune(path, '$') {
					value, err := env.ExpandWithOptions(path, opts)
					if err != nil {
						return err
					}
					path = value
				}

				switch prependPath.Os {
				case "windows":
					if runtime.GOOS == "windows" {
						env.PrependPath(path)
					}
				case "posix":
					if runtime.GOOS == "darwin" || runtime.GOOS == "linux" {
						env.PrependPath(path)
					}
				case "darwin":
					if runtime.GOOS == "darwin" {
						env.PrependPath(path)
					}
				default:
					env.PrependPath(path)
				}

			}
		}
	}

	if len(plan.Dotenv) > 0 {
		for _, file := range plan.Dotenv {
			required := true
			if file[len(file)-1] == '?' {
				file = file[:len(file)-1]
				required = false
			}

			if strings.ContainsRune(file, '$') {
				value, err := env.ExpandWithOptions(file, opts)
				if err != nil {
					return errors.New("failed to expand dotenv variable: " + err.Error())
				}
				file = value
			}

			data, err := os.ReadFile(file)
			if err != nil {
				if required {
					return errors.New("failed to read dotenv file: " + err.Error())
				}
				continue
			}
			nextDoc, err := dotenv.Parse(string(data))
			if err != nil {
				return err
			}
			doc.Merge(nextDoc)
		}
	}

	if len(plan.Env) > 0 {
		for key, value := range plan.Env {
			doc.Set(key, value)
		}
	}

	// params must override the plan's dotenv and env
	if len(params.Dotenv) > 0 {
		for _, file := range params.Dotenv {
			required := true
			if file[len(file)-1] == '?' {
				file = file[:len(file)-1]
				required = false
			}
			if strings.ContainsRune(file, '$') {
				value, err := env.ExpandWithOptions(file, opts)
				if err != nil {
					return err
				}
				file = value
			}

			data, err := os.ReadFile(file)
			if err != nil {
				if required {
					return err
				}
				continue
			}
			nextDoc, err := dotenv.Parse(string(data))
			if err != nil {
				return err
			}
			doc.Merge(nextDoc)
		}
	}

	if len(params.Env) > 0 {
		for key, value := range params.Env {
			doc.Set(key, value)
		}
	}

	envPath := env.Get(env.PATH)

	ctx.Config.Shell = shell
	paths := env.Get("XTASK_PREPEND_PATH")
	if paths != "" {
		if runtime.GOOS == "windows" {
			segs := strings.Split(paths, ";")
			for _, seg := range segs {
				env.PrependPath(seg)
			}
		} else {
			segs := strings.Split(paths, ":")
			for _, seg := range segs {
				env.PrependPath(seg)
			}
		}
	}

	if _, ok := mapEnv["XTASK_ENV"]; !ok {
		f, err := os.CreateTemp("", "xtask-env-")
		if err != nil {
			return err
		}
		f.Write([]byte{})
		f.Close()
		mapEnv["XTASK_ENV"] = f.Name()
		cleanUpEnv = true
	}

	if _, ok := mapEnv["XTASK_PATH"]; !ok {
		f, err := os.CreateTemp("", "xtask-path-")
		if err != nil {
			return err
		}
		f.Write([]byte{})
		f.Close()

		mapEnv["XTASK_PATH"] = f.Name()
		cleanUpPath = true
	}

	defer func() {
		if cleanUpEnv {
			envFile := mapEnv["XTASK_ENV"]
			if envFile != "" {
				os.Remove(envFile)
			}
		}
	}()

	defer func() {
		if cleanUpPath {
			pathFile := mapEnv["XTASK_PATH"]
			if pathFile != "" {
				os.Remove(pathFile)
			}
		}
	}()

	for _, node := range doc.ToArray() {
		if node.Type == dotenv.VARIABLE_TOKEN {
			key := ""

			value := node.Value
			if node.Key != nil {
				key = *node.Key
			}

			key = strings.TrimSpace(key)

			if strings.HasPrefix(key, "XTASK_") {
				if strings.HasSuffix(key, "_EXE") {
					value, err := env.ExpandWithOptions(value, opts)
					if err != nil {
						return err
					}
					env.Set(key, value)
				}

				continue
			}

			value, err := env.ExpandWithOptions(value, opts)
			if err != nil {
				return errors.New("failed to expand " + key + " variable: " + node.Value + " " + err.Error())
			}
			mapEnv[key] = value
		}
	}

	mapEnv[env.PATH] = envPath

	if runtime.GOOS == "windows" {
		gitBin := env.Get("XTASK_GIT_BIN")
		if gitBin == "" {
			gitBin = mapEnv["XTASK_GIT_BIN"]
		}

		if gitBin != "" {
			if _, err := os.Stat(gitBin); err == nil {
				env.AppendPath(gitBin)
			}
		}
	}

	ctx.Schema = plan
	ctx.Tasks = tasks
	ctx.Cwd = filepath.Dir(tasksFile)
	ctx.Env = mapEnv

	err = runTasks(ctx)
	return err
}

func runTasks(ctx WorkflowContext) error {
	taskDefs := ctx.Tasks
	if len(taskDefs) == 0 {
		return errors.New("no tasks to run")
	}

	set := []tasks.Task{}

	// capture path
	envPath := env.Get(env.PATH)

	for _, taskDef := range taskDefs {
		name := taskDef.Id
		if taskDef.Name != nil {
			name = *taskDef.Name
		}
		uses := ""
		if taskDef.Uses != nil {
			uses = *taskDef.Uses
		}

		desc := ""
		if taskDef.Desc != nil {
			desc = *taskDef.Desc
		}

		timeout := time.Duration(0)
		if taskDef.Timeout != nil {
			timeout, _ = time.ParseDuration(*taskDef.Timeout)
		}

		cwd := ctx.Cwd
		if taskDef.Cwd != nil {
			cwd = *taskDef.Cwd
		}

		if cwd == "" {
			cwd = ctx.Cwd
		}

		doc := dotenv.NewDocument()

		envMap := make(map[string]string)

		opts := &env.ExpandOptions{
			CommandSubstitution: ctx.Schema.Config.CommandSubstitution,
			ShellArgs:           ctx.Schema.Dotenv,
			Get: func(key string) string {
				if val, ok := envMap[key]; ok {
					return val
				}
				if val, ok := os.LookupEnv(key); ok {
					return val
				}
				return ""
			},
			Set: func(key, value string) error {
				envMap[key] = value
				return nil
			},
		}

		xenvMap := make(map[string]string)
		for key, value := range ctx.Env {
			envMap[key] = value
		}

		if len(taskDef.Dotenv) > 0 {
			for _, denv := range taskDef.Dotenv {
				required := true
				if denv[len(denv)-1] == '?' {
					denv = denv[:len(denv)-1]
					required = false
				}
				if strings.ContainsRune(denv, '$') {
					value, err := env.ExpandWithOptions(denv, opts)
					if err != nil {
						if required {
							return errors.New("Failed to expand dotenv variable: " + err.Error())
						}
					}
					denv = value
				}

				data, err := os.ReadFile(denv)
				if err != nil {
					if required {
						return errors.New("Failed to read dotenv file: " + err.Error())
					}
					continue
				}
				doc2, err := dotenv.Parse(string(data))
				if err != nil {
					return errors.New("Failed to parse dotenv file: " + err.Error())
				}

				doc.Merge(doc2)
			}
		}

		if len(taskDef.Env) > 0 {
			for key, value := range taskDef.Env {
				doc.Set(key, value)
			}
		}

		if doc.Len() > 0 {

			options := &env.ExpandOptions{
				CommandSubstitution: ctx.Schema.Config.CommandSubstitution,
				ShellArgs:           ctx.Schema.Dotenv,
				Get: func(key string) string {
					if val, ok := envMap[key]; ok {
						return val
					}
					if val, ok := os.LookupEnv(key); ok {
						return val
					}
					return ""
				},
				Set: func(key, value string) error {
					envMap[key] = value
					return nil
				},
			}

			for _, node := range doc.ToArray() {
				if node.Type == dotenv.VARIABLE_TOKEN {
					key := ""
					value := node.Value
					if node.Key != nil {
						key = *node.Key
					}

					if strings.HasPrefix(key, "XTASK_") {
						if strings.HasSuffix(key, "_EXE") {
							value, err := env.ExpandWithOptions(value, options)
							if err != nil {
								return errors.New("Failed to expand environment variable: " + err.Error())
							}
							xenvMap[key] = value
						}

						continue
					}

					value, err := env.ExpandWithOptions(value, options)
					if err != nil {
						return errors.New("Failed to expand environment variable: " + err.Error())
					}
					envMap[key] = value
				}
			}
		}

		envMap[env.PATH] = env.Get(env.PATH)

		run := ""
		if taskDef.Run != nil {
			run = *taskDef.Run
		}

		task := &tasks.Task{
			Id:      taskDef.Id,
			Name:    name,
			Desc:    desc,
			Uses:    uses,
			Env:     envMap,
			XEnv:    xenvMap,
			Run:     run,
			Hosts:   taskDef.Hosts,
			Cwd:     cwd,
			Timeout: timeout,
			Needs:   taskDef.Needs,
			Files:   taskDef.Files,
			Args:    ctx.Args,
		}

		if task.Uses == "" {
			task.Uses = ctx.Config.Shell
		}

		set = append(set, *task)
	}

	for _, task := range set {
		var def schema.TaskDef
		for _, t := range taskDefs {
			if t.Id == task.Id {
				def = t
				break
			}
		}

		envFile := ctx.Env["XTASK_ENV"]
		if len(envFile) > 0 {
			canOpen := true
			if _, err := os.Stat(envFile); err != nil {
				canOpen = false
			}

			if canOpen {
				bytes, err := os.ReadFile(envFile)
				if err != nil {
					return errors.New("Failed to read XTASK_ENV file: " + err.Error())
				}

				if len(bytes) > 0 {
					doc2, err := dotenv.Parse(string(bytes))
					if err != nil {
						return errors.New("Failed to parse XTASK_ENV file: " + err.Error())
					}

					opts := &env.ExpandOptions{
						CommandSubstitution: ctx.Schema.Config.CommandSubstitution,
						ShellArgs:           ctx.Args,
						Get: func(key string) string {
							if val, ok := task.Env[key]; ok {
								return val
							}
							if val, ok := os.LookupEnv(key); ok {
								return val
							}
							return ""
						},
						Set: func(key, value string) error {
							task.Env[key] = value
							return nil
						},
					}

					for _, node := range doc2.ToArray() {
						if node.Type == dotenv.VARIABLE_TOKEN {
							key := ""
							value := node.Value
							if node.Key != nil {
								key = *node.Key
							}

							if strings.HasPrefix("XTASK_", key) {
								if strings.HasSuffix(key, "_EXE") {
									value, err := env.ExpandWithOptions(value, opts)
									if err != nil {
										return errors.New("Failed to expand environment variable: " + err.Error())
									}
									task.XEnv[key] = value
								}

								continue
							}

							value, err := env.ExpandWithOptions(value, opts)
							if err != nil {
								return errors.New("Failed to expand environment variable: " + err.Error())
							}
							task.Env[key] = value
						}
					}
				}
			}
		}

		pathFile := ctx.Env["XTASK_PATH"]
		// reset path
		env.SetPath(envPath)
		if len(pathFile) > 0 {

			canOpen := true
			if _, err := os.Stat(pathFile); err != nil {
				canOpen = false
			}

			if canOpen {
				bytes, err := os.ReadFile(pathFile)
				if err != nil {
					return errors.New("Failed to read XTASK_PATH file: " + err.Error())
				}

				if len(bytes) > 0 {
					content := string(bytes)
					scanner := bufio.NewScanner(strings.NewReader(content))
					for scanner.Scan() {
						line := strings.TrimSpace(scanner.Text())
						if len(line) > 0 {
							if _, err := os.Stat(line); err == nil {
								// LAST IN SHOULD BE FIRST IN PATH
								env.PrependPath(line)
							}
						}
					}
				}
			}
		}

		task.Env[env.PATH] = env.Get(env.PATH)

		c := &tasks.TaskContext{
			Task:    task,
			Targets: ctx.Targets,
			Context: ctx.Context,
			TaskDef: def,
		}

		oldMap := map[string]string{}
		for key, value := range task.XEnv {
			old := env.Get(key)
			oldMap[key] = old
			env.Set(key, value)
		}

		if strings.ContainsRune(task.Cwd, '$') {
			opts := &env.ExpandOptions{
				CommandSubstitution: ctx.Config.CommandSubstitution,
				ShellArgs:           ctx.Args,
				Get: func(key string) string {
					if val, ok := task.Env[key]; ok {
						return val
					}
					if val, ok := os.LookupEnv(key); ok {
						return val
					}
					return ""
				},
				Set: func(key, value string) error {
					task.Env[key] = value
					return nil
				},
			}
			cwd, _ := env.ExpandWithOptions(task.Cwd, opts)
			if cwd == "" {
				cwd = ctx.Cwd
			}
			c.Task.Cwd = cwd
		}

		os.Stdout.WriteString("\x1b[1m" + task.Name + "\x1b[22m\n")
		res := tasks.Run(*c)
		for key, value := range oldMap {
			if value == "" {
				env.Unset(key)
			} else {
				env.Set(key, value)
			}
		}

		if res.Err != nil {
			return res.Err
		}
	}

	return nil
}

func execCommand(plan *schema.Workflow, params Params) {

	doc := dotenv.NewDocument()

	envMap := env.All()
	envMap = normalizeEnvMap(envMap)
	envMap["XTASK_FILE"] = params.File
	envMap["XTASK_DIR"] = filepath.Dir(params.File)
	os.Chdir(filepath.Dir(params.File))
	shell := "bash"
	if plan.Config != nil && plan.Config.Shell != "" {
		shell = plan.Config.Shell
	} else if runtime.GOOS == "windows" {
		shell = "powershell"
	}
	envMap["XTASK_SHELL"] = shell
	opts := &env.ExpandOptions{
		CommandSubstitution: plan.Config.CommandSubstitution,
		Env:                 envMap,
		Get: func(key string) string {
			if val, ok := envMap[key]; ok {
				return val
			}
			if val, ok := os.LookupEnv(key); ok {
				return val
			}

			return ""
		},
		Set: func(key, value string) error {
			envMap[key] = value
			return nil
		},
	}

	if plan.Config != nil {
		if len(plan.Config.Env) > 0 {
			for key, value := range plan.Config.Env {
				next, err := env.ExpandWithOptions(value, opts)
				if err != nil {
					os.Stderr.WriteString("Failed to expand environment variable: " + err.Error() + "\n")
					os.Exit(1)
				}
				envMap[key] = next
			}
		}
	}

	if len(plan.Dotenv) > 0 {
		for _, file := range plan.Dotenv {
			// get last character of file
			required := true
			if file[len(file)-1] == '?' {
				required = false
				file = file[:len(file)-1]
			}

			if strings.ContainsRune(file, '$') {
				value, err := env.ExpandWithOptions(file, opts)
				if err != nil {
					if required {
						os.Stderr.WriteString("Failed to expand dotenv variable: " + err.Error() + "\n")
						os.Exit(1)
					}
				}
				file = value
			}

			data, err := os.ReadFile(file)
			if err != nil {
				if required {
					os.Stderr.WriteString("Failed to read dotenv file: " + err.Error() + "\n")
					os.Exit(1)
				} else {
					continue
				}
			}
			nextDoc, err := dotenv.Parse(string(data))
			if err != nil {
				os.Stderr.WriteString("Failed to parse dotenv file: " + err.Error() + "\n")
				os.Exit(1)
			}
			doc.Merge(nextDoc)
		}
	}

	if len(plan.Env) > 0 {
		for key, value := range plan.Env {
			doc.Set(key, value)
		}
	}

	// params must override the plan's dotenv and env
	if len(params.Dotenv) > 0 {
		for _, file := range params.Dotenv {
			required := true
			if file[len(file)-1] == '?' {
				file = file[:len(file)-1]
				required = false
			}

			if strings.ContainsRune(file, '$') {
				value, err := env.ExpandWithOptions(file, opts)
				if err != nil {
					if required {
						os.Stderr.WriteString("Failed to expand dotenv variable: " + err.Error() + "\n")
						os.Exit(1)
					}
				}
				file = value
			}

			data, err := os.ReadFile(file)
			if err != nil {
				if required {
					os.Stderr.WriteString("Failed to read dotenv file: " + err.Error() + "\n")
					os.Exit(1)
				} else {
					continue
				}
			}
			nextDoc, err := dotenv.Parse(string(data))
			if err != nil {
				os.Stderr.WriteString("Failed to parse dotenv file: " + err.Error() + "\n")
				os.Exit(1)
			}
			doc.Merge(nextDoc)
		}
	}

	if len(params.Env) > 0 {
		for key, value := range params.Env {
			doc.Set(key, value)
		}
	}

	for _, node := range doc.ToArray() {
		if node.Type == dotenv.VARIABLE_TOKEN {
			key := ""
			value := node.Value
			if node.Key != nil {
				key = *node.Key
			}

			value, err := env.ExpandWithOptions(value, opts)
			if err != nil {
				os.Stderr.WriteString("Failed to expand environment variable: " + err.Error() + "\n")
				os.Exit(1)
			}
			envMap[key] = value
		}
	}

	cmd := &exec.Cmd{
		Cmd: ex.CommandContext(params.Context, params.Args[0], params.Args[1:]...),
	}

	cmd.Dir = filepath.Dir(params.File)
	cmd.WithEnvMap(envMap)

	o, err := cmd.Run()
	if err != nil || o.Code != 0 {
		if o.Code != 0 {
			os.Exit(o.Code)
		} else {
			os.Exit(1)
		}
	}

	os.Exit(o.Code)
}

func normalizeEnvMap(envMap map[string]string) map[string]string {
	if runtime.GOOS == "windows" {
		envMap["HOME"] = env.Get(env.HOME)
		envMap["USER"] = env.Get(env.USER)
		envMap["HOSTNAME"] = env.Get(env.HOSTNAME)
		envMap["OSTYPE"] = "windows"
	}

	envMap["OS_PLATFORM"] = runtime.GOOS
	envMap["OS_ARCH"] = runtime.GOARCH

	if !env.Has(env.HOME_CONFIG) {
		dir, _ := os.UserConfigDir()
		if dir != "" {
			envMap[env.HOME_CONFIG] = dir
		}
	}

	if !env.Has(env.HOME_CACHE) {
		dir, _ := os.UserCacheDir()
		if dir != "" {
			envMap[env.HOME_CACHE] = dir
		}
	}

	if !env.Has(env.HOME_DATA) {
		switch runtime.GOOS {
		case "windows":
			localAppData := env.Get("LOCALAPPDATA")
			if localAppData != "" {
				envMap[env.HOME_DATA] = localAppData
			} else {
				envMap[env.HOME_DATA] = filepath.Join(env.Get(env.HOME), "AppData", "Local")
			}
		case "darwin":
			envMap[env.HOME_DATA] = filepath.Join(env.Get(env.HOME), "Library", "Application Support")
		default:
			envMap[env.HOME_DATA] = filepath.Join(env.Get(env.HOME), ".local", "share")
		}
	}

	return envMap
}
